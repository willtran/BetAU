			...
			<tbody>		
					@foreach($users as $key => $user)
						<tr id="user_{{ $user->id }}" @if($key%2) class="event_background" @else class="odd_background" @endif >							
							...
							<td align="center">
								<a class="item_edit" href="{{ URL::route('admin.user.edit',array('user' => $user->id)) }}" ><img src="{{ asset('/images/dashboard/blank.png') }}" alt="User Edit" title="Edit user"/></a>
								<a class="item_delete" href="javascript:void(0);" onclick="user.userDelete('{{ URL::route('admin.user.destroy', array('user'=>$user->id)) }}',{{ $user->id }});"><img class="user_delete" src="{{asset('images/dashboard/blank.png')}}" alt="User Delete" title="Delete user"/></a>
							</td>
						</tr>
					@endforeach
